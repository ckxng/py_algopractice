...
class TestRedBlack(unittest.TestCase):
    ...
    def test_get_node_color(self):
        tree = _create_tree()
        self.assertEqual(redblack._get_node_color(tree.left.right), redblack.BLACK)
    ...
    def test_left_rotation(self):
        ...
        tree = _create_tree()
        self.assertRaises(redblack.NullNodeError, redblack._left_rotation, tree.left.right)  # not possible

    def test_find_node_by_key(self):
        tree = _create_tree()
        self.assertEqual(redblack._find_node_by_key(tree, 700).key, 700)  # findable
        self.assertRaises(KeyError, redblack._find_node_by_key, tree, 750)  # not findable
        self.assertRaises(KeyError, redblack._find_node_by_key, None, 700)  # no tree provided
